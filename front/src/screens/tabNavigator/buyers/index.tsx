...
export default function Buyers({navigation}: BuyersProps) {
  ...
  const [toggleFavorite] = useGetBuyerLikePostMutation();
  const [deletePost] = useDeleteRequirementPostMutation();
  const [toggleLike] = useTogglePostLikeMutation();

  const fetchBuyerPosts: LazyFetcher<PostListDatum> = async param => {
    const formData = transformQueryParam(param);
    const result = await trigger({data: formData, type: 'buyer'}, false);
  ...
  const controller = usePaginatedEntityList<PostListDatum & {id: number}>(
    fetchBuyerPosts as LazyFetcher<PostListDatum & {id: number}>,
    {
  ...
  const initiateChat = useCallback(
    async (postInfo: PostListDatum) => {
      onInitiateChat(postInfo, userInfo ?? {}, 'buyer');
    },
    [userInfo],
  );
  ...
  const onPressShare = useCallback(() => {
    onSharePost('buyer', selectedPost?.id?.toString() ?? '', '', {
  ...
  return (
    <Container>
      ...
      <View style={[VS.flex_1, VS.gap_15]}>
        <HeaderComponent
          postTitle={'Tell sellers what you want'}
          withFilter
          searchInput={query}
          onSearch={onChangeText}
          onAddPost={() => {
            navigation.navigate('AddPostRequirement', {
              requestFrom: 'buyer' as RequestTypeFromParam,
              onGoBack: () => {
                controller?.refresh();
              },
            });
