...
//                            $query->where('user_id', '!=', $user->id);
                    ...
                    if (!empty($states_id)) {
                        $query->where(function ($q) use ($states_id) {
                            foreach ($states_id as $stateIdd) {
                                $q->orWhereRaw("FIND_IN_SET(?, register.state_id) > 0", [$stateIdd]);
                            }
                        });
                    }
                }
            ...
            if (!empty($latitude) && !empty($longitude)) {
                ...
                $query->orderBy('register.name', 'asc');
            }
            $data = $query->get();
...
//        try {
        ...
        $working_with_data->transform(function ($items) {
            if ($items->user_image) {
                ...
                $items->user_image = "https://soundwale.in/public/admin-asset/images/profile_default_image.png";
            }
            return $items;
        });
