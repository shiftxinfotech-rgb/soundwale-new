...
class SellerDetailsController extends Controller {
    ...
    public function seller_list_get(Request $request) {
        ...
        if (!empty($role_id)) {
            ...
            if (!empty($roles_id)) {
                $query->where(function ($q) use ($roles_id) {
                    ...
                    $q->orWhereNull('seller_details.role_id');
                });
            }
        }
        if ($state_name != "") {
                ...
                if (!empty($states_id)) {
                    $query->where(function ($q) use ($states_id) {
                        foreach ($states_id as $stateIdd) {
                            $q->orWhereRaw("FIND_IN_SET(?, seller_details.state_id) > 0", [$stateIdd]);
                        }
                        $q->orWhereNull('seller_details.state_id');
                    });
                }
            }
        }
        ...
        if (!empty($latitude) && !empty($longitude)) {
            ...
            $query->orderBy('seller_details.id', 'desc');
        }
        $data = $query->get();
...
//            SellerDetailsImages::where('seller_details_id', $list_inserted_Id)->delete();
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $file) {
                $path = Helper::uploadImageWithOrientation($file, SellerDetailsImages::IMAGE_PATH);
                $imageRecords[] = [
                    'user_id' => $user->id,
