...
class BuyerRequirmentController extends Controller {
    ...
    public function buyer_list_get(Request $request) {
        ...
        if (!empty($role_id)) {
            ...
            if (!empty($roles_id)) {
                $query->where(function ($q) use ($roles_id) {
                    ...
                    $q->orWhereNull('buyer_requirment.role_id');
                });
            }
        }
        if ($state_name != "") {
                    ...
                    if (!empty($states_id)) {
                        $query->where(function ($q) use ($states_id) {
                            foreach ($states_id as $stateIdd) {
                                $q->orWhereRaw("FIND_IN_SET(?, buyer_requirment.state_id) > 0", [$stateIdd]);
                            }
                            $q->orWhereNull('buyer_requirment.state_id');
                        });
                    }
                }
            } 
        ...
        if (!empty($latitude) && !empty($longitude)) {
            ...
            $query->orderBy('buyer_requirment.id', 'desc');
        }
        $data = $query->get();
...
//            }
            foreach ($request->file('images') as $file) {
                $path = Helper::uploadImageWithOrientation($file, BuyerRequirmentImages::IMAGE_PATH);
                $imageRecords[] = [
                    'user_id' => $user->id,
