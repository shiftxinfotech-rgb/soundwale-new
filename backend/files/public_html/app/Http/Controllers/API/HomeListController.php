...
//        ->whereRaw('FIND_IN_SET(?, home_list.role_id)', [$user->role_id]);
//        ->where('home_list.user_id', '!=', $user->id);
        ...
        if (!empty($role_id)) {
            ...
            $query->whereRaw('FIND_IN_SET(?, home_list.role_id)', [$user->role_id]);
        }
        if ($state_name != "") {
            ...
            if (!empty($states_id)) {
                $query->where(function ($q) use ($states_id) {
                    foreach ($states_id as $stateId) {
                        $q->orWhereRaw("FIND_IN_SET(?, home_list.state_id) > 0", [$stateId]);
                    }
                });
            }
        } else {
            $query->whereRaw('FIND_IN_SET(?, home_list.state_id)', [$user->state_id]);
        }
    ...
    public function home_list_add(Request $request) {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $file) {
                $path = Helper::uploadImage($file, HomeListImages::IMAGE_PATH);
                $imageRecords[] = [
                    'user_id' => $user->id,
